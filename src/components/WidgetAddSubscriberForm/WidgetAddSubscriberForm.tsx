...
const WidgetAddSubscriberForm: FC<Props> = ({
	...
	return (
		<div className={`nc-WidgetAddSubscriberForm overflow-hidden ${className}`}>
			<WidgetHeading1
				title={NC_SITE_SETTINGS.subcription_widget?.title || 'Subscribe'}
				icon={<EnvelopeIcon className="h-5 w-5 flex-none" />}
			/>
			<div className="p-4 xl:p-5">
				<span
					className="mt-2 text-sm leading-6 text-neutral-600 dark:text-neutral-400"
					dangerouslySetInnerHTML={{
						__html: NC_SITE_SETTINGS.subcription_widget?.description || '',
					}}
				></span>
				<div className="mt-4">
					<AddSubscriberForm />
